...
{
    /// <summary>
    /// UI type editor for <see cref="ILocFunctions"/>.
    /// </summary>
    internal class LocFunctionsEditor : UITypeEditor
    {
        /// <summary>
        ...
        {
            var source = value as ILocFunctions;
            var settings = context.GetFirstEntitySettings<IEntitySettings>();
            if ((settings == null) || (source == null))
                return value;
            using (var dialog = new LocFunctionsEditorForm(source, settings.Context))
            {
                dialog.ShowDialog();
            }
            return source;
