...
// Put a loc on the track
func (s *service) PutLocOnTrack(ctx context.Context, req *api.PutLocOnTrackRequest) (*api.RailwayState, error) {
	rwState, err := s.getRailwayState()
	if err != nil {
		return nil, err
	}
	...
	if err := locState.Enable(ctx); err != nil {
		return nil, err
	}
	var result api.RailwayState
	if err := result.FromState(ctx, s.railwayState); err != nil {
