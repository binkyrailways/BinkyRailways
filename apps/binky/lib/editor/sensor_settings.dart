...
class _SensorSettingsState extends State<_SensorSettings> {
  ...
  Widget build(BuildContext context) {
    return Column(
      children: <Widget>[
        Text(widget.sensor.id),
        SettingsTextField(
            controller: _descriptionController,
            label: "Description",
            firstChild: true,
            onLostFocus: (value) async {
              final sensor = await widget.model.getSensor(widget.sensor.id);
              var update = sensor.deepCopy()..description = value;
              widget.model.updateSensor(update);
            }),
      ],
    );
  }
